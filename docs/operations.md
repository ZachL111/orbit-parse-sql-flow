...
`orbit-parse-sql-flow` is centered on parsers work in Ruby. The technical target is: Implement a Ruby parsers project for sql state machine modeling, using transition tables and invalid-transition tests.
...
The score is deterministic. The current threshold is 176. Risk has a penalty of 5, latency has a penalty of 3, and weight contributes a bonus of 4.
